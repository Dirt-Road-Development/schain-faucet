...
const MINIMUM_BALANCE: BigNumber = ethers.utils.parseEther('0.000000005');
const MAXIMUM_BALANCE: BigNumber = ethers.utils.parseEther('0.000005')
...
export const DotController = async(req: Request, res: Response) => {
    try {
        ...
        /// 2 -> Check if DOT
        const isDot: boolean = await isDOT(provider.provider, req.params['address']);
        if (!isDot) return res.status(400).send('This is not a DOT');
        /// 2 -> If DOT, Check DOT Balance
